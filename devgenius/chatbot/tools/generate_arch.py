"""
Herramienta: Generar diagrama de arquitectura AWS (XML draw.io).
Exporta el archivo .drawio a la carpeta output/.
"""
import os
import json
import datetime
...
@tool
def generate_architecture(conversation_context: str) -> str:
    """
    Genera un diagrama de arquitectura AWS en formato XML (draw.io) basado en el contexto
    de la conversación. Usa esta herramienta cuando el usuario pida generar un diagrama
    de arquitectura visual de la solución.

    Args:
        conversation_context: Resumen de la solución/arquitectura discutida en la conversación.

    Returns:
        El XML del diagrama generado y la ruta del archivo exportado.
    """
    from chatbot.config import BEDROCK_MODEL_ID, BEDROCK_MAX_TOKENS
    import boto3
    from chatbot.config import AWS_REGION
